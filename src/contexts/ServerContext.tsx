...
export const ServerProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchServers = useCallback(
    async (silent = false) => {
      // Prevent fetching servers if user is not authenticated yet
      // This stops 401 Unauthorized errors on public pages like Landing
      if (!user) {
        setLoading(false);
        setServers([]);
        return;
      ...
      try {
        ...
        if (data.length > 0) {
          ...
          if (user?.activeServer) {
            targetServer = data.find(
              (s: Server) => s._id === user.activeServer,
            );
          }
